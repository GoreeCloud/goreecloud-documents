...
func NewLocalOriginalStore(root string) (*LocalOriginalStore, error) {
	if root == "" {
		return nil, fmt.Errorf("original store root is required")
	}
...
func (s *LocalOriginalStore) Put(_ context.Context, key string, src io.Reader, maxBytes int64) (int64, string, error) {
	if src == nil || maxBytes <= 0 {
		return 0, "", fmt.Errorf("source and positive size limit are required")
	}
	path, err := s.path(key)
	if err != nil {
		return 0, "", err
	...
	if _, err := os.Stat(path); err == nil {
		return 0, "", fmt.Errorf("original already exists")
	} else if !os.IsNotExist(err) {
		return 0, "", err
	}
	tmp, err := os.CreateTemp(filepath.Dir(path), ".original-*")
	if err != nil {
		return 0, "", err
	}
	name := tmp.Name()
	...
	if n > maxBytes {
		return 0, "", fmt.Errorf("original exceeds configured maximum")
	}
	...
	if err := os.Link(name, path); err != nil {
		return 0, "", fmt.Errorf("publish original: %w", err)
	}
...
func (s *LocalOriginalStore) path(key string) (string, error) {
	if len(key) != 36 {
		return "", fmt.Errorf("storage key must be a canonical UUID")
	}
	for i, r := range key {
